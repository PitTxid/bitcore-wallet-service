...
V8.prototype.translateQueryAddresses = function(addresses) {
  ...
  return BCHAddressTranslator.translate(addresses, this.addressFormat, 'copay', this.coin);
};
...
V8.prototype.translateResultAddresses = function(addresses) {
  ...
  return BCHAddressTranslator.translate(addresses, 'copay', this.addressFormat, this.coin);
};
...
V8.prototype.initSocket = function(callbacks) {
  ...
  socket.on('coin', (data) => {
    ...
    var out;
    try {
      let addr = self.coin == 'bch' ? BCHAddressTranslator.translate(data.address, 'copay', 'cashaddr', self.coin) : data.address;
      out = { 
        address: addr,
